...
if __name__ == '__main__':
    ...
    parser.add_argument('--pair_id', type=int, nargs='+', help="Specify pairs 5, 6, 7, ...")
    ...
    if args.pair_id:
        # Process only the specified pairs
        print(f"Processing pairs: {', '.join(map(str, args.pair_id))}")
        pair_ids = args.pair_id
    else:
        # Process all pairs as per the original logic
    ...
    if args.device == 'cpu' and args.n_jobs > 1 and not args.pair_id:
        # Multiprocessing only for the full dataset
        with Pool(args.n_jobs) as p:
            res_list = list(p.imap_unordered(process_id, pair_ids))
    else:
        # Sequential processing for a single pair or small datasets
